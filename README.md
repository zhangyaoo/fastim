...
3. 对于离线用户，用户上线后，对比最新的消息ID和last_ack_msg_id，来进行拉取
...
## 五、Q&A
### 1 架构
1. Q:消息ID为什么是趋势递增就可以，为什么不是严格递增的？
...
5. Q:MQ的使用场景？
   ...
   A:IM消息是非常庞大的，比如说群聊、推送，对于一些业务上可以忍受的场景，尽量使用MQ来解耦和通信，来降低同步通讯的服务器压力。
   
6. Q:群消息存一份还是多份，读扩散还是写扩散？

   A:存1份，读扩散。存多份下同一条消息存储了很多次，对磁盘和带宽造成了很大的浪费。可以在架构上和业务上进行优化，来实现读扩散。

7. Q:为什么消息入库后，通知客户端来拉取，而不是直接推送给客户端？

   A:为了统一客户端获取消息的方式，目的是简化同一时刻客户端同时拉取和同时接收消息推送处理的复杂性。
   
8. Q:为什么除了单聊 群聊，其他的业务都走http协议？
